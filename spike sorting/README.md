...
3. Afterward, we can plot the selected principal components in the principal component space, which enables intuitive visualization and classification of the spike snippets. Furthermore, by applying algorithms such as k-means to the principal components, it can automatically group the snippets into clusters corresponding to different types of neurons.

4. The final step in spike sorting is to assign the rasters to one neuron or another.
